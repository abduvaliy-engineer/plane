...
export function InputColorPicker(props: InputColorPickerProps) {
  ...
  // Recover by locating the trigger button from this component's own root.
  React.useEffect(() => {
    if (isOpen && !referenceElement && rootRef.current) {
  ...
  // portaled here).
  React.useEffect(() => {
    if (isOpen && !popperElement && rootRef.current) {
      const el = rootRef.current.querySelector<HTMLDivElement>(
        '[data-headlessui-state~="open"] > div, [data-open] > div'
      );
  ...
  return (
    <div className="relative">
      ...
      <Popover as="div" ref={rootRef} className="absolute top-1/2 right-1 z-10 -translate-y-1/2">
        {({ open }) => {
